...
| Codebase Memory structural exploration | exact `codebase-memory-mcp` 0.10.1 | No Codebase Memory tools, capability, prompt, or grep augmentation; cloud logs a nonfatal hard error | Commit-pinned, digest-verified official installer into Felan agent storage with upstream configuration disabled |

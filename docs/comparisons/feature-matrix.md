...
| Feature | Felan Code | Codex | OpenCode | Claude Code | Pi | Oh My Pi |
| --- | --- | --- | --- | --- | --- | --- |
| Startup project instructions | At most one file in the session cwd: `AGENTS.md`, then `CLAUDE.md`; delivered as hidden user-role context outside the system prompt | Global instructions plus one `AGENTS.override.md`, `AGENTS.md`, or fallback file per directory from repo root to cwd | Global file plus ancestor project files; `AGENTS.md` wins as a filename family over `CLAUDE.md` | Managed, user, project, and local `CLAUDE.md` files; does not read `AGENTS.md` unless imported | Global and ancestor-to-cwd `AGENTS.override.md`, `AGENTS.md`, or `CLAUDE.md` | Discovers native and foreign instruction formats from OMP, Claude, Codex, Gemini, OpenCode, Copilot, and other locations |
...
| Local non-interactive/API mode | **Ships:** `felan --mode text` prints one final response; `--mode json` emits Pi-compatible JSONL; native stable ACP v1 runs over guarded stdio with isolated sessions, load/replay, terminal auth, elicitation, and permissions | `codex exec` with text or JSONL events; local app-server protocol | `opencode run`, local server, SDK/OpenAPI, and ACP | `claude -p` output formats and Agent SDK | Print, JSON, RPC, and Node SDK | Print, RPC/RPC-UI, Node SDK, and ACP |
...
| Browser automation | **Conditional:** reviewed `agent-browser` 0.37.1 CLI integration with trusted cross-origin HTTP(S) browser_authorize attachment, configurable consent, and screenshot policy | No core browser tool in the reviewed CLI | Plugin/custom tool required | **Integration:** separate Claude-in-Chrome component | **Integration:** required | **Ships:** headless/attached Chrome and Electron/CDP; existing-tab control uses a browser relay integration |
...
### RTK and background processes are separate features
...
- Persistent user-configured local stdio MCP outside ACP in Codex, OpenCode,
  Claude Code, and OMP.
...
- **Felan Code:** local release candidate `0.24.4` in the current repository state.
  ...
  [MCP][felan-mcp], [native ACP][felan-acp], [Codex tools][felan-codex],
  [browser][felan-browser], and [structured questions][felan-ask].
...
[felan-acp]: ../user-guide/local-cli.md#native-acp-v1

...
> `@earendil-works/pi-*` packages at `0.85.1`.
...
Felan Code is a host and extension layer built on that foundation. Choose Felan Code when
...
| TUI | Local Felan Code groups tool activity, exposes task/agent/process overlays, and owns the footer/presentation | Pi TUI primitives and default host presentation |
...
Felan Code's local TUI owns credentials, settings, storage path mapping, model scope,
built-in allowlisting, dependency onboarding, local subagent execution, and
presentation. It filters ambient Pi resources and exposes only explicit Felan Code
...
approval system. The tradeoff is assembly work: Pi gives you primitives; Felan Code
gives you a maintained local composition.

### Felan Code's task graph and Prewalk are product workflows

Pi's core does not provide Felan Code's dependency-aware task state. Felan Code's tasks
...
### Felan Code adds explicit external-content boundaries
...
Felan Code consumes pinned Pi packages and re-exports the shared Pi composition
...
- you want explicit host/runtime/security ownership across local and managed
  deployments; or
- you want Pi's provider/session foundation with Felan Code's product layer on top.
...
- Felan Code's fixed local resource boundary is too narrow for your application; or
- you want the smallest foundation and are comfortable owning integrations and
  lifecycle behavior yourself.

## Migration and interoperability

Felan Code uses the pinned Pi extension and session APIs, but it is not a drop-in
...
Felan Code documents and tests a narrower local policy, but the local host still has
...
See [Felan Code architecture](../concepts/architecture.md),
...
### Is Felan Code a Pi fork?

No. Felan Code wraps pinned `@earendil-works/pi-*` packages and composes them with
its own Agent Core, host runtime, and extensions.

### Can I use a Pi extension unchanged?

Not automatically in the local TUI. Felan Code loads only source-controlled built-
...
No. Agent Core exposes the pinned Pi provider/model composition surface. Felan Code

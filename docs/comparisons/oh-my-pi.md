...
Felan Code is a good fit when you want a portable Agent Core and host-owned policy,
...
large configurable extension ecosystem. The central tradeoff is Felan Code's
portable, narrower host boundary versus OMP's breadth and native integration.

## At a glance

| Dimension | Felan Code | Oh My Pi |
| --- | --- | --- |
...
| Code intelligence | No built-in LSP/DAP in the reviewed Felan Code local surface | In-process LSP and native DAP tools |
...
desktop automation, GitHub/path schemes, and collaboration. Felan Code's local
...
not describe Felan Code as a replacement for those OMP capabilities; they are real
OMP strengths in the reviewed baseline.

### Felan Code's task graph is a different coordination primitive

Felan Code puts hard prerequisites, ready/blocked state, worker claims, acceptance
...
workspaces. Felan Code is simpler when the primary coordination problem is “which
...
Codex, Cline, Windsurf, Gemini, Copilot, and other configuration shapes. Felan Code
...
file and explicit Felan Code agents/skills under its own host policy.

OMP therefore has a lower migration cost for a repository built around those
ecosystems. Felan Code has a smaller implicit resource surface and a clearer reason
for every loaded built-in.

### Both build on Pi lineage, but with different ownership choices

Felan Code wraps pinned `@earendil-works/pi-*` packages and keeps host policy and
...
allows OMP to own a wide in-process tool/runtime surface; Felan Code keeps Pi as a
...
Felan Code and OMP can share ordinary repository files, but their resource discovery
policies are intentionally different. Felan Code does not load OMP plugins,
marketplaces, foreign skills, or OMP configuration automatically. OMP's broad
discovery should not be assumed to make Felan Code a drop-in replacement.

Felan Code's `.mcp.json` support is remote HTTP OAuth only and host-owned; OMP's MCP
...
Felan Code's local host runs unsandboxed with current-user permissions, while its
...
See [Felan Code runtime and security](../concepts/runtime-and-security.md), the
...
### Can Felan Code run OMP extensions or plugins?

Not automatically. Felan Code's local TUI loads source-controlled built-ins only.
Port behavior into a Felan extension or compose a compatible package through an
explicit host integration.

### Does Felan Code have OMP's LSP or debugger?

Not in the reviewed local surface. Felan Code's comparison is intentionally candid:
OMP is the better fit when LSP, DAP, or native debugger workflows are required.

### Why choose Felan Code if both use Pi-related packages?

Felan Code's value is its ownership boundary: the portable Agent Core and extensions

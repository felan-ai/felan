...
Felan Code is a good fit when you want a portable Agent Core, explicit source-
...
| Code intelligence | No built-in LSP or debugger in Felan Code's reviewed local surface | LSP is available but reviewed defaults/configuration can leave it disabled; no DAP core feature |
...
### Felan Code's task graph is one execution state model

Felan Code combines task identity, prerequisites, ready/blocked queries, atomic
...
checklist and independent workers are enough, but it does not express Felan Code's
...
its plan workflow. Felan Code Prewalk does not create a read-only plan gate: it keeps
...
integration, and configurable permissions. Felan Code's ordinary models use Pi's
small coding surface and commonly search through shell commands; RTK can
compact command-aware results, but Felan Code does not currently provide OpenCode's
first-class LSP surface.

This is a meaningful OpenCode advantage for repositories where diagnostics,
symbol navigation, or structured file search are central. Felan Code's strength is
...
user/project environment. Felan Code's local TUI filters ambient Pi extensions,
packages, prompts, themes, project settings, and package resources and loads
only its source-controlled built-ins plus explicit Felan Code skills/agents.

Felan Code therefore has a smaller implicit attack and compatibility surface, but
...
- first-class `glob`, `grep`, and LSP are more valuable than Felan Code's narrower
  coding surface;
...
- OpenCode's server, SDK, or TUI surfaces are a better fit than Felan Code's local
  interactive binary.

## Migration and interoperability

Both systems can work with repository instruction files, but their discovery
and configuration rules differ. Felan Code reads one cwd-level `AGENTS.md` or
...
Felan Code does not import OpenCode plugins, project settings, MCP configuration,
...
Felan Code's local host is unsandboxed but has explicit boundaries for web SSRF,
...
security posture. Read [Felan Code runtime and security](../concepts/runtime-and-security.md)
...
### Can Felan Code load my OpenCode plugins?
...
### Does Felan Code replace OpenCode's LSP?

No. Felan Code's reviewed local surface does not ship a built-in LSP tool. Use
...
Only selectively. Felan Code supports explicit remote HTTP OAuth servers through its

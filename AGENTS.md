...
- New extensions intended to reduce model or provider costs must include a representative benchmark before merge. The benchmark must compare the extension-enabled configuration with an equivalent disabled baseline on verified tasks, report quality/correctness first plus cost, tokens, and latency, and preserve reproducible cases, methodology, and results as repository artifacts or linked documentation. Do not claim savings from token reduction alone.

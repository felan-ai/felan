...
- Resolve `xhigh`, `high`, `medium`, and `low` model selectors through Agent Core using the session's allowed authenticated models and current-provider preference before sending exact model references to the host. Reserve `xhigh` for unusually complex architecture, design, planning, debugging, and high-stakes review work.

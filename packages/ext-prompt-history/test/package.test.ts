...
describe('@felan-ai/ext-prompt-history package boundary', () => {
  it('publishes metadata and source attribution', async () => {
    ...
    expect(manifest).toMatchObject({ name: '@felan-ai/ext-prompt-history', dependencies: { '@earendil-works/pi-tui': '0.85.1' }, peerDependencies: { '@felan-ai/agent-core': '^0.7.0' }, publishConfig: { access: 'public', provenance: true } });

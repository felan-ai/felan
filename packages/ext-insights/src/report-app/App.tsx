...
function toComputedSession(session: Analytics['sessions'][number]): ComputedSession {
  return {
    ...
    activity: session.activity?.map(item => ({ ...item, timestamp: new Date(item.timestamp) })),
  };
}
...
function filterAnalyticsByDate(analytics: Analytics, range: DateRange): Analytics {
  const sessions = analytics.sessions
    ...
    .map(session => sliceParsedSession(session, range.start ? localDayStart(range.start) : undefined, range.end ? localDayStart(addDays(range.end, 1)) : undefined))
...
function SavingsTab({ report }: { report: InsightsSavingsReport | undefined }) {
  ...
  return <>
    ...
    <div className="section-subtitle">Estimated API-equivalent cost avoided by Felan Code optimizations.</div>
  ...
  return (
    <div className="container">
      ...
      <div className="footer">
        Generated by Felan Code Insights · {allData.export?.generatedAt ? new Date(allData.export.generatedAt).toLocaleString() : 'unknown time'} · static snapshot; run /insights again to refresh
      </div>
    </div>
  );
}

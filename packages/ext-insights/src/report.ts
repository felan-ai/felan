...
export function renderReport(analytics: Analytics): string {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light dark"><meta name="theme-color" content="#255f44"><link rel="icon" type="image/svg+xml" href="${REPORT_FAVICON}"><title>Felan Code Insights</title><style>${REPORT_APP_STYLE}</style><script>${themeBootstrap}</script></head><body><div id="root"></div><script>window.__FELAN_INSIGHTS_DATA__=${data};</script><script>${REPORT_APP_SCRIPT.replace(/<\//gu, '<\\/')}</script></body></html>`;
}

...
function isSkillReadPath(filePath: string, options: Pick<ToolResultCompactionOptions, 'cwd' | 'agentDir'>): boolean {
  if (!filePath.trim()) {
    return false;
  }

  const resolvedPath = normalizeRuntimePath(filePath, options.cwd);
  const explicitRoots = [joinRuntimePath(options.agentDir, 'skills'), joinRuntimePath(options.cwd, '.pi', 'skills')];
  if (explicitRoots.some((root) => isRuntimePathUnderRoot(resolvedPath, root, options.cwd))) {
    return true;
  }

  return resolvedPath.includes('/.agents/skills/');
}
...
function shouldPreserveExactReadOutput(
  ...
  options: Pick<ToolResultCompactionOptions, 'cwd' | 'agentDir'>,
): boolean {
  ...
  if (config.outputCompaction.preserveExactSkillReads && isSkillReadPath(normalizePath(input), options)) {
    return true;
  }
...
function shouldApplyReadSourceFiltering(text: string, config: RtkOptimizerConfig): boolean {
...
function applyAnsiStripping(state: CompactionState, compaction: RtkOptimizerConfig['outputCompaction']): void {
...
function beginCompaction(
  text: string,
  config: RtkOptimizerConfig,
): { state: CompactionState; compaction: RtkOptimizerConfig['outputCompaction'] } {
...
function compactCodexCommandText(
  text: string,
  command: string | undefined,
  config: RtkOptimizerConfig,
): { text: string; techniques: string[] } {
...
function compactGrepText(text: string, config: RtkOptimizerConfig): { text: string; techniques: string[] } {
  const { state, compaction } = beginCompaction(text, config);
...
export function compactToolResult(
  ...
  const { changed, content: nextContent } = mapTextContentBlocks(sourceContent, (contentBlock) => {
    let transformed = { text: contentBlock.text, techniques: [] as string[] };
    if (isCommandToolName(event.toolName)) {
      ...
      transformed = compactReadText(
        ...
        normalizedPath,
        config,
        shouldPreserveExactReadOutput(contentBlock.text, input, config, options),
      );
    ...
    return transformed.text !== contentBlock.text ? transformed.text : undefined;
  });

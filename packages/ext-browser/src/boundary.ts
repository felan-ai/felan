...
export const BROWSER_CAPABILITY_INSTRUCTION = [
  ...
  'Before attaching to an existing browser, profile, or saved authentication state, use browser_authorize and provide a concise reason explaining the authenticated task to the user; its local policy handles consent and may be configured to ask or always allow. After authorization, the attached browser exposes the native agent-browser command surface.',
